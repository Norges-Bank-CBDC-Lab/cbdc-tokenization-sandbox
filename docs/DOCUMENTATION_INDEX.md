...
- `docs/plans/jupyter-removal-plan.md`: Execution-ready removal plan for the JupyterHub-based script runner — PR 1 moves the shared contract-metadata configmaps out of the `jupyterhub` namespace (live-verified: on default deploys that namespace exists only to carry them), PR 2 removes runtime plumbing, gateway listener, pins, the service code, notebook CI, and license-inventory entries atomically. Replacement UI is out of scope (NB UI covers bank/central-bank flows; equity flows retire to the ERC-3643 track).
