...
- `docs/plans/backend-design-improvements-backlog.md`: Ranked NB Bond API architecture backlog. Read-your-writes coordination, projection-first Bond/Auction reads, the projection-purity rule, and lifecycle-truthful bond statuses have shipped; preserved-table migrations, treasury-held-unit handling, universal preflight simulation, and product naming remain follow-ups.
- `docs/plans/operator-audit-trail-design.md`: Historical design brief for the persistent NB Bond API operator audit trail. The implemented form records all operator mutation types in preserved `operation_attempts` storage and exposes the global Operations page; the per-bond merged GET proposed by the brief was deferred. Implementation evidence is in `docs/plans/archive/operator-audit-trail-plan.md` (PR #213).
