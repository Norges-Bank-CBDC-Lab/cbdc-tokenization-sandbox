...
- `docs/decisions/README.md`: Architecture Decision Records (ADRs) — index of immutable, point-in-time records of architecturally significant decisions (one decision per file; superseded, never rewritten).
  ...
  - `docs/decisions/0004-settle-all-bond-cash-legs-in-wnok.md`: Why every bond cash leg (issuance, buyback, coupon, redemption) settles in `Wnok` from a government reserve account, removing the government-nominated `Tbd` from the bond stack while keeping the allowance-based authorization for the closed-loop plan to replace. Accepted.
  - `docs/decisions/0005-redeem-principal-with-the-final-coupon.md`: Why the final coupon payment also pays principal, burns every unit (unsold manager-held units without cash), and emits one `BondMatured` closure event, removing the separate redemption step and the treasury-held-units deadlock. Accepted.
- `docs/plans/README.md`: How implementation plans are laid out — new plans are folders with `intent.md`, `design.md`, `plan.md`, and a committed `progress.md`; existing single-file plans stay as they are.
...
- `docs/plans/archive/`: Reference-only archive of completed implementation plans. Status lines inside each plan record the implementing PR and its state where relevant:
  - `docs/plans/archive/blockscout-catchup-indexer/`: Plan folder that enabled Blockscout's catch-up indexer so blocks mined while it was stopped, broken, or deleted are indexed after it starts, the genesis block included; records the live proof (genesis, a component-stop gap filled within seconds, a full re-index from block 0 after `blockscout.sh delete`) and two follow-ups found along the way (BENS names missing from Blockscout's API, a BENS startup race on a fresh namespace). Shipped with the change itself.
  - `docs/plans/archive/sandbox-stop-keeps-state/`: Plan folder that made `stop` keep all state and `delete` the only verb that discards it: `./sandbox.sh stop` stops the Kind node gracefully and `start` resumes it, component `stop` verbs scale to zero while new `delete` verbs carry the old uninstall behaviour, the NB Bond API shuts down on SIGTERM, and every `helm` and `kubectl` call names the sandbox's context; records the live stop/start evidence, the 90-second systemd wait inside the Kind node, and the Blockscout PostgreSQL recovery. Shipped with the change itself.
  - `docs/plans/archive/better-sqlite3-13-upgrade/`: Plan folder that moved the NB Bond API to `better-sqlite3` 13.0.3 (Node-API addon, prebuilt binaries shipped in the package, `prebuild-install` and its subtree removed, `node-addon-api` added) with a classified lockfile transplant and the inventory row; records the node-gyp step that compiles only without a matching prebuilt binary, and the local-sandbox evidence that the rebuilt image loads the linux binary and that a resync from block 0 reproduces the projection and keeps the system-of-record tables. Shipped with the change itself.
  - `docs/plans/archive/dependabot-rollup-2026-09-25/`: Plan folder that landed four Dependabot bumps (zod 4.6.5, typescript-eslint 8.70.1 with its siblings, `@vitejs/plugin-react` 6.1.1, `@testing-library/react` 16.3.3) as one rollup with the inventory rows and a classified lockfile transplant; OpenAPI output unchanged, and the `better-sqlite3` 13 major left for its own PR. Shipped with the change itself.
  - `docs/plans/archive/dependabot-rollup-2026-09/`: Plan folder that landed the five open non-security Dependabot bumps (typescript-eslint 8.69.0 with its siblings, `@types/node` 26.5.1 with the Node types pin, `@testing-library/jest-dom` 7.0.1, zod 4.5.4, prettier 3.9.6) as one rollup with the inventory rows and a classified lockfile transplant; records the OpenAPI nullable-shape change from zod and the pre-existing typescript-eslint peer gap. Shipped with the change itself.
  - `docs/plans/archive/dependabot-security-rollup-2026-09/`: Plan folder that closed Dependabot alerts #78–#81 with one lockfile change (root `js-yaml` override to 4.3.2; nb-ui `vitest` and its `@vitest/*` siblings to 4.1.11), transplanted with a structural diff and override audit; records the constructed `js-yaml` entry and the excluded incidental `tinyrainbow` bump. Shipped via #281.
  - `docs/plans/archive/coupon-closure-review-fixes/`: Plan folder that fixed the review findings on the wNOK cash-leg and maturity-closure work: integer coupon preview and BigInt reserve check in the UI, `withdrawFailedIssuance` removed, zero-supply bonds closable through the coupon route, duplicate holders rejected on-chain and in the body, a per-period `CouponPeriodPaid` event driving the projected count, a `BondManager` compatibility flag in health, and two replay fixes the fresh-sandbox run exposed. Shipped via #278.
  - `docs/plans/archive/redeem-with-final-coupon/`: Plan folder (intent, design, plan, progress) that made the final coupon payment close the bond: coupon plus principal in wNOK per holder, every unit burned, unsold manager-held units skipped on coupons and burned at maturity without payment, one `BondMatured` event with totals, and the separate redemption contract function, API route, and UI client removed; includes the fresh-sandbox balance trails for a fully allocated and a failed-allocation bond. Decision in ADR 0005. Shipped via #275 and #276.
  - `docs/plans/archive/bond-cash-leg-wnok/`: Plan folder (intent, design, plan, progress) that moved bond coupon, buyback, and redemption cash from the government-nominated TBD to wNOK paid from the government reserve account, so primary dealers buy and are paid in central-bank money; covers `BondManager`, deploy wiring, tests, the Central Bank API field and UI, live-change publication, and the two related known issues, with the fresh-sandbox balance trail in `progress.md`. Decision in ADR 0004. Shipped via #272 and #273.
  - `docs/plans/archive/browserslist-dev-transitive-refresh-plan.md`: Lockfile-only refresh of the development-scoped transitive `browserslist` (4.28.2 → 4.28.8, Dependabot alert #76) together with the five sibling packages it requires; no manifest or override change, gated by the structural lockfile diff and override audit. Shipped via #263.
  ...
  - `docs/plans/archive/dependency-security-pin-refresh-plan.md`: Security refresh of the root `overrides` floors (js-yaml 4.3.1, undici 7.29.0, scoped brace-expansion 1.1.18/5.0.9) plus runtime ip-address/body-parser transitives via a full-resolve lockfile transplant with a structural-diff gate, clearing all 14 Dependabot alerts; the override-bump procedure it documents stays referenced from `docs/KNOWN_ISSUES.md`. Shipped via #247; released in v0.7.1.
...
- `services/blockscout/debugging.md`: Blockscout/Besu debugging playbook, including recovery of a PostgreSQL that will not start after an unclean shutdown.
