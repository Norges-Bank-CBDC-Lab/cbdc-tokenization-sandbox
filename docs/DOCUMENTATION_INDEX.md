...
- `docs/decisions/README.md`: Architecture Decision Records (ADRs) — index of immutable, point-in-time records of architecturally significant decisions (one decision per file; superseded, never rewritten).
  ...
  - `docs/decisions/0005-redeem-principal-with-the-final-coupon.md`: Why the final coupon payment also pays principal, burns every unit (unsold manager-held units without cash), and emits one `BondMatured` closure event, removing the separate redemption step and the treasury-held-units deadlock. Proposed.
...
- `docs/plans/redeem-with-final-coupon/`: Plan folder (intent, design, plan, progress) to close a bond in one transaction: the final `payCoupon` pays coupon plus principal in wNOK, burns all units, skips and burns unsold manager-held units, emits `BondMatured`, and removes the separate redemption contract function, API route, and UI client; covers projection status, UI payout preview with a reserve-balance warning, sandbox validation, and docs. Decision in ADR 0005.
