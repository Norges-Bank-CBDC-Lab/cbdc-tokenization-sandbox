...
- `docs/plans/bond-lifecycle-management-plan.md`: Bond lifecycle management — pre-stage bonds, soft-delete via `disablePartition`, standalone create + disable endpoints (backlog items 11 + 12). Functional code shipped via #127; Phase 5 documentation + archival outstanding — see `docs/plans/outstanding-plan-items.md`.
- `docs/plans/outstanding-plan-items.md`: Triage list of leftover / deferred items extracted from partially-completed or shipped plans (bond-lifecycle Phase 5 docs, one optional auction UI test) — for the operator to fold into other plans or drop.
