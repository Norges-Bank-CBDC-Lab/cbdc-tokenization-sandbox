...
- `docs/plans/dependabot-rollup-2026-09/`: Plan folder (intent, design, plan, progress) to land the five open non-security Dependabot bumps (typescript-eslint, `@types/node` with the Node types pin, `@testing-library/jest-dom` 7, zod, prettier) as one rollup with the inventory rows, a classified lockfile transplant, and the tool-consequence checks (reformat, lint, OpenAPI regeneration, test suites), then close the bot PRs.
