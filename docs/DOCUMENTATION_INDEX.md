...
- `docs/plans/sse-live-updates-plan.md`: Authenticated push notifications via Server-Sent Events — a bearer/recognized-role-protected `GET /v1/events` combines post-commit ingestion notifications with successful request-path mutation notifications. One `fetch()` + `ReadableStream` client per tab works with both local `none` mode and the deployed Entra/MSAL flow, triggering ETag-preserving live-query reloads and coarse reconnect reconciliation. `/v1/health` stays independent; Azure buffering/timeouts and single-replica delivery are explicit deployment requirements; projection/replay remain deferred.
- `docs/plans/cursor-reconcile-sync-plan.md`: Superseded design note. Its transport-agnostic `useLiveQuery` reconciliation shape was absorbed by `sse-live-updates-plan.md`; the separate `/v1/health` cursor trigger was not implemented.
- `docs/plans/nb-application-architecture-improvements-plan.md`: Phased modular-monolith improvements for NB Bond API and NB UI — fixes auction/integer correctness and explicit read-failure semantics first, then aligns API/auth/SSE contracts, extracts a testable Express app and feature services, improves read consistency and background-refresh UX, and splits Zod/OpenAPI contracts by feature without new packages or production-scale infrastructure.
- `docs/plans/projection-aligned-api-contract-plan.md`: Follow-up implementation plan for checkpoint-consistent Bond/Auction DTOs and feature-owned Zod/OpenAPI contracts — fixes balance reducer double-application and unstable ETags first, then projects bond/auction state, adds honest post-commit `202` handling, replaces misleading lifecycle states, and completes the contract split without new infrastructure.
