...
- `docs/plans/dependabot-security-rollup-2026-09/`: Plan folder (intent, design, plan, progress) to close Dependabot alerts #78–#81 with one lockfile change: the root `js-yaml` override to 4.3.2 and the nb-ui `vitest` devDependency to 4.1.11 (with its seven `@vitest/*` siblings), transplanted into the lockfile with a structural diff and override audit as the gate; the non-security Dependabot PRs stay separate.
