...
- `docs/plans/coupon-closure-review-fixes/`: Plan folder (intent, design, plan, progress) fixing the review findings on the wNOK cash-leg and maturity-closure work: integer coupon preview and BigInt reserve check in the UI, `withdrawFailedIssuance` removed, zero-supply bonds closable through the coupon route, duplicate holders rejected on-chain and in the body, a per-period `CouponPeriodPaid` event driving the projected count, a `BondManager` compatibility flag in health, and the related cleanup.
