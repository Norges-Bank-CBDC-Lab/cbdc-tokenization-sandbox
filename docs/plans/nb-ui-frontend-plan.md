...
4. ~~**`nb-bond-api` still uses the older host-mount + pod-side-`npm-build` deploy.**~~ **Done (post-merge, follow-up branch `feature/nb-bond-api-image-baked`).** `services/nb-bond-api/Dockerfile` + a rewritten `deployNBBondAPI` build/push the image with a content-hash tag, the chart switched to the image-baked shape with an `emptyDir` at `/app/data`, and both per-service Kind `extraMounts` (`nb-bond-api` and the already-dead `nb-ui` one) were removed from `infra/cluster/cluster-config.yaml`. The same branch also dropped the `USE_KIND_REGISTRY` flag and the `kind load docker-image` fallback in `common/helpers.sh`, since the registry is now a hard prerequisite for every code path that ships.
