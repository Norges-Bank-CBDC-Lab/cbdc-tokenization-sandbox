...
- Before running the contracts flow directly, generate the local fixture files:
...
- The generated local bidder keys live in
  `scripts/bid-submitter/examples/bids.keys.json`, and the runnable encrypt
  inputs live under `.tmp/bid-encryption/examples/`.
...
npm run submit --sealed-bids ../../.tmp/bid-encryption/examples/basic/sealed.json \
