...
- Env: generate `contracts/.env` with `node scripts/generate-local-sandbox-fixtures.mjs` before direct runs if the start script has not already created it
...
- Env: `contracts/.env` (local-only; generated by `node scripts/generate-local-sandbox-fixtures.mjs` or created manually from `contracts/.env.example`)
