...
```bash
# Encrypt a plaintext payload to file (optionally with signing params embedded in input JSON)
...
npm run decrypt examples/basic/unseal.auctioneer.example.json ./opened.json

# Decrypt a ciphertext as bidder to console
npm run decrypt examples/basic/unseal.bidder.example.json
...
Two of the three addresses supplied as bidders will result in successful bids - Nordea (0x15d) and DNB (0x996). A third bidder
is also supplied but will result in a failed DvP at finalisation.
