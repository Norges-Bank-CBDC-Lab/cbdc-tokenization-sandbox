...
import {
  marginalClearingRate,
  selectAuctionBidPositions,
} from '../src/domain/auctionAllocation.js';
...
describe('auction allocation preview', () => {
  it('returns the highest selected rate when the selection fits within the offering (RATE)', () => {
    ...
    expect(marginalClearingRate(picked, 'RATE', '2000')).toBe(425n);
  });
  ...
  it('returns the marginal (lower) rate when the selection over-fills the offering (RATE)', () => {
    ...
    expect(marginalClearingRate(picked, 'RATE', '200')).toBe(325n);
  });
  ...
  it('fills highest-first for PRICE auctions', () => {
    ...
    expect(marginalClearingRate(picked, 'PRICE', '5')).toBe(100n);
    expect([...selectAuctionBidPositions(picked, 'PRICE', '5')]).toEqual([0, 1]);
  });
  ...
  it('breaks ties on rate by larger units first', () => {
    ...
    expect(marginalClearingRate(picked, 'RATE', '3')).toBe(100n);
    expect([...selectAuctionBidPositions(picked, 'RATE', '3')]).toEqual([1]);
  });

  it('returns 0 for an empty selection', () => {
    expect(marginalClearingRate([], 'RATE', '2000')).toBe(0n);
  });

  it('does not lose precision above Number.MAX_SAFE_INTEGER', () => {
    const picked = [
      bid('9007199254740993', '9007199254740993', 0),
      bid('9007199254740995', '1', 1),
    ];
    expect(marginalClearingRate(picked, 'PRICE', '1')).toBe(9007199254740995n);
    expect([...selectAuctionBidPositions(picked, 'PRICE', '1')]).toEqual([1]);
  });
});
