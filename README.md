...
4. Create the required local-only config files from the tracked examples:
...
Edit the placeholder values in those local files before continuing. They are
for local sandbox use only and must never be reused outside local development.
